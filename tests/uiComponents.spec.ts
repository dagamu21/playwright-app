...
test.describe('Form Layouts Page', () => {
    test.describe.configure({retries: 2}) //override config to retry
    test.beforeEach( async({page}) => {
    ...
    test('input fields', async({page}) => {
        ...
        await usingTheGridEmailInput.pressSequentially('test@teste.com', {delay: 500})

        //generic assertion
        const inputValue = await usingTheGridEmailInput.inputValue()
        expect(inputValue).toEqual('test@teste.com')

        //locator assertion
        await expect(usingTheGridEmailInput).toHaveValue('test@teste.com')
    })
...
test('web tables', async({page}) => {
    ...
    //get the row based on the value of a specific column
    ...
    for(let age of ages){
        ...
        const ageRows = page.locator('tbdoy tr')
...
test('datepicker', async({page}) => {
    ...
    let calendarMonthYeat = await page.locator('nb-calendar-view-mode').textContent()
    const expectedMonthAndYear = ` ${expectedMonthLong} ${expectedYear} `
    while(!calendarMonthYeat.includes(expectedMonthAndYear)){
        await page.locator('nb-calendar-pageable-navigation [data-name="chevron-right"]').click()
        calendarMonthYeat = await page.locator('nb-calendar-view-mode').textContent()
...
test('vota ninel', async({page}) => {
    await page.goto('https://vix.com/es-es/external-login?type=vote&embed=core')
    //await page.waitForTimeout(500)
    //const signIn = page.locator('input[type="email"], input[name="email"], input#email')

    // await page.locator('input[type="email"]').isVisible();
    // await page.locator('input[type="password"]').isVisible();

    //const frame = page.frameLocator('[class="flowics-viz"] iframe')
    //await page.waitForTimeout(5000)
    //await frame.locator('[class="container container-size-medium"]').filter({hasText: "¡Vota para salvar a tu favorito!"}).waitFor()

    const email= page.getByPlaceholder('ejemplo@email.com')
